...
export function AuthForm({ mode }: { mode: "login" | "signup" }) {
  ...
  const submit = async (e: FormEvent) => {
    ...
    setError("");
    const em = email.trim();
    if (!em || password.length < 6) return setError("Email and a 6+ char password are required.");
    if (mode === "signup" && !fullName.trim()) return setError("Full name is required.");
    setBusy(true);
    try {
      if (mode === "login") await login(em, password);
      else await signup(fullName.trim(), em, password);
      ...
      <div className="w-full max-w-sm flex flex-col gap-5">
        ...
        <form onSubmit={submit} className="flex flex-col gap-3">
          ...
          <button type="submit" disabled={busy}
            className="rounded-md bg-primary text-primary-foreground px-4 py-2.5 text-sm font-medium hover:bg-primary/90 disabled:opacity-60">
...
function GoogleIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 48 48" aria-hidden>
      <path fill="#FFC107" d="M43.6 20.5H42V20H24v8h11.3C33.7 32.4 29.3 35.5 24 35.5c-6.3 0-11.5-5.2-11.5-11.5S17.7 12.5 24 12.5c2.9 0 5.6 1.1 7.6 2.9l5.7-5.7C33.8 6.4 29.1 4.5 24 4.5 13.2 4.5 4.5 13.2 4.5 24S13.2 43.5 24 43.5c11 0 19.5-8 19.5-19.5 0-1.3-.1-2.3-.4-3.5z"/>
      ...
      <path fill="#4CAF50" d="M24 43.5c5 0 9.6-1.9 13.1-5l-6-5c-2 1.4-4.4 2.2-7.1 2.2-5.3 0-9.7-3.1-11.3-7.5l-6.5 5C9.6 39.5 16.2 43.5 24 43.5z"/>
      <path fill="#1976D2" d="M43.6 20.5H42V20H24v8h11.3c-.8 2.2-2.3 4.1-4.2 5.5l6 5c-.4.4 6.4-4.7 6.4-14.5 0-1.3-.1-2.3-.4-3.5z"/>
    </svg>
  );
}
